...
> 自动生成于 2026-05-31T03:22:35.870Z
> 共 23 个 skills
...
> **触发场景**: Creating algorithmic art using p5.js with seeded randomness and interactive parameter exploration. Use this when users request creating art using code, generative art, algorithmic art, flow fields, or particle systems. Create original algorithmic art rather than copying existing artists' work to avoid copyright violations.
...
> **触发场景**: Applies Anthropic's official brand colors and typography to any sort of artifact that may benefit from having Anthropic's look-and-feel. Use it when brand colors or style guidelines, visual formatting, or company design standards apply.
...
> **触发场景**: Create beautiful visual art in .png and .pdf documents using design philosophy. You should use this skill when the user asks to create a poster, piece of art, design, or other static piece. Create original visual designs, never copying existing artists' work to avoid copyright violations.
...
> **触发场景**: 处理文件编码安全问题的约束指南。当涉及任何文件读写操作（尤其是在 PowerShell / Windows 环境下操作 JS、Vue、MD、JSON、配置文件等源码文件）时，必须使用此 skill。触发关键词：编码、BOM、UTF-8、PowerShell 读写文件、Set-Content、Get-Content、文件乱码、文件头字节、脚本写回文件。

# UTF-8 编码约束

## 核心原则

仓库中所有源码和文档默认使用 **UTF-8 无 BOM** 编码。任何文件读写操作都必须显式指定编码，绝不依赖系统默认编码。

---

## 读文件

在 PowerShell 中读取源码、文档、配置文件时，**必须显式指定 `-Encoding utf8`**：

```powershell
# ✅ 正确
Get-Content -LiteralPath 'docs\frd\wode-design.md' -TotalCount 140 -Encoding utf8

# ❌ 错误：依赖 PowerShell 默认编码，可能导致乱码
Get-Content -LiteralPath 'docs\frd\wode-design.md' -TotalCount 140
```
...
**禁止**对 JS、Vue、MD 等源码文件使用 PowerShell 5.x 的 `Set-Content -Encoding utf8`，因为它会写入 BOM（`EF BB BF`）。

```powershell
# ❌ 绝对禁止：PowerShell 5.x 的 utf8 会写入 BOM
Set-Content -Path 'mock/data/activity.js' -Encoding utf8 -Value $content
```

如果必须通过脚本写回文件，统一使用 .NET 的 `System.Text.UTF8Encoding($false)`：

```powershell
# ✅ 正确：UTF-8 无 BOM
$utf8NoBom = New-Object System.Text.UTF8Encoding($false)
[System.IO.File]::WriteAllText(
    (Resolve-Path 'mock/data/activity.js'),
    $content,
    $utf8NoBom
)
```
...
```powershell
# 读取前 4 个字节
$bytes = [System.IO.File]::ReadAllBytes('mock/data/activity.js') | Select-Object -First 4
$bytes | ForEach-Object { $_.ToString('X2') }
```

判断规则：
- 开头是 `EF BB BF` → 文件带有 BOM，需要处理
- 开头无 `EF BB BF` → 正常的 UTF-8 无 BOM

修复带 BOM 文件：

```powershell
$utf8NoBom = New-Object System.Text.UTF8Encoding($false)
$content = Get-Content -LiteralPath 'mock/data/activity.js' -Encoding utf8 -Raw
[System.IO.File]::WriteAllText(
    (Resolve-Path 'mock/data/activity.js'),
    $content,
    $utf8NoBom
...
`mock/data/activity.js` 曾被 PowerShell 误写成 `UTF-8 with BOM`，后续编辑该文件时需格外注意。
...
- [ ] 是否使用了 `Set-Content -Encoding utf8`？→ 改用 `System.Text.UTF8Encoding($false)`
- [ ] 是否显式指定了读取编码？→ 加上 `-Encoding utf8`
- [ ] 写回后是否需要验证文件头？→ 检查首字节不为 `EF BB BF`

---

## 🔧 Skill: doc-coauthoring

> **触发场景**: Guide users through a structured workflow for co-authoring documentation. Use when user wants to write documentation, proposals, technical specs, decision docs, or similar structured content. This workflow helps users efficiently transfer context, refine content through iteration, and verify the doc works for readers. Trigger when user mentions writing docs, creating proposals, drafting specs, or similar documentation tasks.
...
> **触发场景**: A set of resources to help me write all kinds of internal communications, using the formats that my company likes to use. Claude should use this skill whenever asked to write some sort of internal communications (status reports, leadership updates, 3P updates, company newsletters, FAQs, incident reports, project updates, etc.).
...
## 🔧 Skill: java-interviewer

> **触发场景**: 模拟 Java 高级工程师 / 架构师技术面试。当用户说"开始面试"、"Java 面试"、"帮我练习面试"、"模拟面试官"，或上传了简历/代码/技术文档并希望进行面试时，必须使用此技能。支持无文件标准面试和基于简历/代码/文档的针对性面试两种模式。
...
> **触发场景**: Guide for creating high-quality MCP (Model Context Protocol) servers that enable LLMs to interact with external services through well-designed tools. Use when building MCP servers to integrate external APIs or services, whether in Python (FastMCP) or Node/TypeScript (MCP SDK).
...
### 📁 可用脚本（skills/mcp-builder/scripts/）
...
> **触发场景**: Use this skill whenever the user wants to do anything with PDF files. This includes reading or extracting text/tables from PDFs, combining or merging multiple PDFs into one, splitting PDFs apart, rotating pages, adding watermarks, creating new PDFs, filling PDF forms, encrypting/decrypting PDFs, extracting images, and OCR on scanned PDFs to make them searchable. If the user mentions a .pdf file or asks to produce one, use this skill.
...
If you need to fill out a PDF form, first check to see if the PDF has fillable form fields. Run this script from this file's directory:
 `python skills/pdf/scripts/check_fillable_fields <file.pdf>`, and depending on the result go to either the "Fillable fields" or "Non-fillable fields" and follow those instructions.

# Fillable fields
If the PDF has fillable form fields:
- Run this script from this file's directory: `python skills/pdf/scripts/extract_form_field_info.py <input.pdf> <field_info.json>`. It will create a JSON file with a list of fields in this format:
```
[
  {
...
`python skills/pdf/scripts/extract_form_structure.py <input.pdf> form_structure.json`
...
`python skills/pdf/scripts/convert_pdf_to_images.py <input.pdf> <images_dir/>`
...
`python skills/pdf/scripts/convert_pdf_to_images.py <output.pdf> <verify_images/>`
...
> **触发场景**: Use this skill whenever the user asks to write, review, fix, or explain PowerShell scripts. Triggers include: any mention of "PowerShell", ".ps1" files, PowerShell cmdlets (Get-Item, Set-Location, etc.), Windows automation tasks, batch file operations, system administration scripts, or cross-platform scripting with PowerShell Core. Also triggers when the user asks to move, copy, rename, delete files/directories using PowerShell, or asks about execution policy, error handling, or PowerShell modules.

# PowerShell 开发规范与最佳实践

## 概述

本规范适用于所有 PowerShell 脚本开发，涵盖命名规范、代码格式、安全性、错误处理及跨平台兼容性。生成的脚本必须严格遵守以下规则。

---

## 命名规范

### 变量命名 — PascalCase 或 camelCase
...
.SYNOPSIS
    处理用户数据并生成报告
.DESCRIPTION
    读取用户数据，计算统计信息，生成 HTML 报告
.PARAMETER Path
    用户数据文件的路径
.EXAMPLE
    Process-UserData -Path "C:\data\users.csv"
#>
...
### 标准模式：try-catch + ErrorAction Stop
...
```powershell
# 忽略单个命令的错误
Move-Item -Path $source -Destination $dest -ErrorAction SilentlyContinue

# 在循环中忽略错误
Get-ChildItem -Path "C:\source" | ForEach-Object {
    Move-Item -Path $_.FullName -Destination "D:\dest\" -ErrorAction SilentlyContinue
}
```
...
```powershell
# 设置 UTF-8 编码（处理中文路径必须）
$OutputEncoding = [System.Text.Encoding]::UTF8
[Console]::OutputEncoding = [System.Text.Encoding]::UTF8

# 使用 -LiteralPath 处理包含 [ ] 等特殊字符的路径
...
```powershell
Get-ChildItem                                          # 当前目录
Get-ChildItem -Path "C:\target" -Force                 # 包含隐藏文件
Get-ChildItem -Path "C:\target" -Recurse               # 递归
Get-ChildItem -Path "C:\target" -Filter "*.md"         # 按类型筛选
```
...
```powershell
Copy-Item -Path "C:\src\file.txt" -Destination "C:\dst\file.txt" -Force
Copy-Item -Path "C:\src\dir"     -Destination "C:\dst\dir"      -Recurse
```

### 移动

```powershell
Move-Item -Path "C:\src\file.txt" -Destination "C:\dst\file.txt" -Force
Move-Item -Path "C:\src\*.txt"    -Destination "C:\dst\"
Move-Item -Path "C:\src\*"        -Destination "C:\dst\"
```

### 删除

```powershell
Remove-Item -Path "C:\file.txt"              # 文件
Remove-Item -Path "C:\dir" -Recurse          # 目录
Remove-Item -Path "C:\file.txt" -Force       # 强制（不提示）
```
...
- [ ] 变量和函数命名遵循 PascalCase / 动词-名词格式
- [ ] 使用 `Join-Path` 构建路径，不拼接字符串
- [ ] 关键操作使用 `try-catch` + `-ErrorAction Stop`
- [ ] 无硬编码密码或敏感信息
- [ ] 参数使用 `[ValidateNotNullOrEmpty()]` 等验证属性
- [ ] 中文路径已设置 UTF-8 编码
- [ ] 批量删除/移动操作前使用 `-WhatIf` 预览
- [ ] 函数提供 `.SYNOPSIS` / `.DESCRIPTION` 注释块
- [ ] 跨平台代码使用 `$IsWindows` / `$IsLinux` 检测

---

## 🔧 Skill: pptx

> **触发场景**: "Use this skill any time a .pptx file is involved in any way — as input, output, or both. This includes: creating slide decks, pitch decks, or presentations; reading, parsing, or extracting text from any .pptx file (even if the extracted content will be used elsewhere, like in an email or summary); editing, modifying, or updating existing presentations; combining or splitting slide files; working with templates, layouts, speaker notes, or comments. Trigger whenever the user mentions \"deck,\" \"slides,\" \"presentation,\" or references a .pptx filename, regardless of what they plan to do with the content afterward. If a .pptx file needs to be opened, created, or touched, use this skill."
...
> **触发场景**: 基于本项目 python-rules 规范的 FastAPI 全栈开发技能。 触发场景：新增/修改 API 路由、SQLModel 数据库模型、CRUD 操作、统一响应封装、 路由注册、后台任务与日志、前端 HTML 模板、pytest 接口测试。 凡涉及 app/api、app/models、app/schemas、app/service、tests 目录的开发均应加载本技能。
...
## 🔧 Skill: quotation-docx

> **触发场景**: 生成专业的中文项目报价单 Word 文档（.docx）。当用户提到"报价单"、"报价"、"开发报价"、"项目报价"、"做一个报价"、"生成报价文档"时，立即使用此 skill。支持任意项目类型（软件开发、小程序、网站、设计、硬件等），自动套用品牌主色、模块明细表、费用汇总、付款方式、里程碑计划等标准结构，输出专业可发送的 .docx 文件。即使用户只说"帮我出个报价"也应触发此 skill。
...
### 📁 可用脚本（skills/quotation-docx/scripts/）

- `skills/quotation-docx/scripts/template_60k.js`
- `skills/quotation-docx/scripts/template_full.js`

---

## 🔧 Skill: skill-creator

> **触发场景**: Create new skills, modify and improve existing skills, and measure skill performance. Use when users want to create a skill from scratch, update or optimize an existing skill, run evals to test a skill, benchmark skill performance with variance analysis, or optimize a skill's description for better triggering accuracy.
...
└── Bundled Resources (optional)
    ├── skills/skill-creator/scripts/    - Executable code for deterministic/repetitive tasks
...
### 📁 可用脚本（skills/skill-creator/scripts/）
...
> **触发场景**: Knowledge and utilities for creating animated GIFs optimized for Slack. Provides constraints, validation tools, and animation concepts. Use when users request animated GIFs for Slack like "make me a GIF of X doing Y for Slack."
...
## 🔧 Skill: tech-review-writer

> **触发场景**: 撰写开源 AI 工具的深度评测公众号文章。当用户提供一个开源项目（名称、GitHub 链接、README 或相关描述），需要写成深度测评、项目分析、技术博文、公众号文章时，立即使用此 skill。触发关键词包括：写评测、写博文、公众号文章、项目分析、写一篇关于 XX 的文章、帮我分析这个项目、深度测评等。即使用户只说"帮我写这个项目的介绍"也应触发此 skill。

# 开源 AI 工具深度评测写作 Skill

## 角色定位

你是一名科技公众号作者，擅长写开源 AI 工具的深度评测文章。语气像「一个懂技术的朋友在聊天」，专业但不装，有独立判断，不写 PR 稿。

## 写作风格要求

- **语气**：口语化但不失专业，像朋友聊天。禁用"首先""综上所述""不得不说""值得一提的是""总的来说"等套话
- **立场**：不只说好话。有亮点就夸，有硬伤就直说，带出自己的判断，不当项目的 PR 机器
- **技术术语**：出现时用一句话解释，但不过度解释——读者有一定技术背景
- **数据**：具体真实，宁可不写也不要编造；如果信息不足，用 [待补充] 标注
- **排版**：句子短，段落短，每段 3-5 行

## 文章结构模板

章节标题格式：`01 / 02 / 03` 编号，标题简短有力。

### 01 开头钩子（50-100字）
用反差、故事或有趣的背景切入，建立情绪或好奇心。**不要**直接介绍项目，先让读者想知道"然后呢"。

### 02 它为什么存在
- 这个项目是谁做的？出发点是什么？
- 解决了什么真实痛点？
- 换成自己的理解说，**不要**复述 README 的营销语言
...
### 07 争议或问题（不能省）
**必须写**。明显缺陷、社区争议、宣传与实测不符的地方——直说，不要绕。
如果没有明显问题，也要写出潜在风险或局限性。

### 08 最后说说
平衡判断：亮点是什么、问题是什么、适合什么样的人用。
**不要**给满分，**不要**全盘否定，**不要**加运营引导语（"欢迎关注""点赞在看"等）。
...
- 数字用阿拉伯数字，不用中文数字
- 代码/命令用 \`反引号\` 包裹
- 如果信息不足以写某节，用 [待补充：需要 XX 信息] 标注，继续写其他部分

## 执行方式

用户给出项目信息后，**直接开始写**，不问多余问题。如果信息明显不足，在文章开头加一行说明，然后基于已有信息写完全文。

信息不足的优先级判断：
1. 有 GitHub README → 可以写完整文章
2. 只有项目名 → 先网络搜索补充信息，再写
3. 有具体使用体验描述 → 重点写实用性部分

---

## 🔧 Skill: theme-factory

> **触发场景**: Toolkit for styling artifacts with a theme. These artifacts can be slides, docs, reportings, HTML landing pages, etc. There are 10 pre-set themes with colors/fonts that you can apply to any artifact that has been creating, or can generate a new theme on-the-fly.
...
> **触发场景**: Suite of tools for creating elaborate, multi-component claude.ai HTML artifacts using modern frontend web technologies (React, Tailwind CSS, shadcn/ui). Use for complex artifacts requiring state management, routing, or shadcn/ui components - not for simple single-file HTML/JSX artifacts.
...
### 📁 可用脚本（skills/web-artifacts-builder/scripts/）
...
> **触发场景**: Toolkit for interacting with and testing local web applications using Playwright. Supports verifying frontend functionality, debugging UI behavior, capturing browser screenshots, and viewing browser logs.
...
### 📁 可用脚本（skills/webapp-testing/scripts/）
