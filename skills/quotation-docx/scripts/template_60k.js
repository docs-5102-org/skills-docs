    ...
    children: [
      ...
      new Table({
        ...
        rows: [
          ...
          new TableRow({ children: [
            ...
            new TableCell({ borders, width:{size:1300,type:WidthType.DXA}, shading:{fill:"F0E0E0",type:ShadingType.CLEAR}, margins:{top:80,bottom:80,left:80,right:80}, children:[new Paragraph({alignment:AlignmentType.CENTER,children:[new TextRun({text:"5,000",size:18,font:"Arial"})]})]}),
          ]}),

          // --- 附近设备 ---
          dataRow(["2","附近设备 & 地图","LBS定位查询附近20台设备\n地图标注（腾讯地图SDK）\n设备状态显示（营业中/维护中）\n搜索设备功能","6","7","3,800"], [420,3000,3086,900,1200,1300], false),

          // --- 扫码点单 ---
          dataRow(["3","扫码点单 & 商品选购","扫码识别设备二维码\n商品分类展示 & 规格选择\n自定义口味/温度/甜度\n购物车 & 数量管理","8","10","5,300"], [420,3000,3086,900,1200,1300], true),

          // --- 支付 ---
          dataRow(["4","支付 & 优惠券","微信支付对接\n优惠券抵扣（无门槛/满减）\n次卡购买 & 核销\n支付结果回调处理","8","12","6,200"], [420,3000,3086,900,1200,1300], false),
          ...
          new TableRow({ children: [
            ...
            new TableCell({ borders, width:{size:1300,type:WidthType.DXA}, shading:{fill:primaryColor,type:ShadingType.CLEAR}, margins:{top:100,bottom:100,left:80,right:80}, children:[new Paragraph({alignment:AlignmentType.CENTER,children:[new TextRun({text:"54,000",bold:true,size:22,color:"FFFFFF",font:"Arial"})]})]}),
          ]}),
        ]
      }),
      ...
      new Table({
        ...
        rows: [
          ...
          dataRow(["功能开发费","54,000","含全部9个模块"], [3302,3302,3302], false),
          dataRow(["UI设计费","3,000","参考卡法未来品牌视觉规范"], [3302,3302,3302], true),
          dataRow(["测试 & 部署","2,000","含上线支持、小程序审核配合"], [3302,3302,3302], false),
          dataRow(["运维支持（首年）","1,000","Bug修复，云服务不含"], [3302,3302,3302], true),
          new TableRow({ children: [
            ...
            new TableCell({ borders, columnSpan:2, width:{size:6604,type:WidthType.DXA}, shading:{fill:primaryColor,type:ShadingType.CLEAR}, margins:{top:100,bottom:100,left:120,right:120}, children:[new Paragraph({alignment:AlignmentType.CENTER,children:[new TextRun({text:"￥60,000 元（人民币陆万元整）",bold:true,size:24,color:"FFFFFF",font:"Arial"})]})]}),
          ]}),
        ]
      }),
      ...
      new Table({
        ...
        rows: [
          ...
          dataRow(["首款","30%","18,000","合同签订后3个工作日内"], [1600,2000,3306,3000], false),
          dataRow(["中期款","40%","24,000","完成核心功能提测验收后"], [1600,2000,3306,3000], true),
          dataRow(["尾款","30%","18,000","小程序上线验收通过后"], [1600,2000,3306,3000], false),
        ]
      }),
...
Packer.toBuffer(doc).then(buffer => {
  fs.writeFileSync("/mnt/user-data/outputs/咖啡机小程序开发报价单_6万预算版.docx", buffer);
  console.log("Done");
});
